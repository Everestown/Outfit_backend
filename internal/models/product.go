...
type Product struct {
    ...
    ProductCode string    `gorm:"type:varchar(5);uniqueIndex;not null" json:"product_code"`
...
type Size struct {
    ID        uint      `gorm:"primaryKey" json:"id"`
    Name      string    `gorm:"type:varchar(25);not null;unique" json:"name" validate:"required"`
    CreatedAt time.Time `json:"created_at"`
    UpdatedAt time.Time `json:"updated_at"`
}
...
type Category struct {
    ID        uint      `gorm:"primaryKey" json:"id"`
    Name      string    `gorm:"type:varchar(50);not null;unique" json:"name" validate:"required"`
